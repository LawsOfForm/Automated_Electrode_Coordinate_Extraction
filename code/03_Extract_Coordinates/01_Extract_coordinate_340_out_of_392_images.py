...
def find_nifti_files(base_path):
    #pattern = os.path.join(base_path, "sub-*", "unzipped", "*inference_reduced_network.nii.gz")
...
def process_nifti_files(base_path, log: Logger):
    ...
    counters = {
        ...
        # Files whose subject/session/run could not be parsed at all
        'unresolved_id':                    0,
        'unresolved_id_sub':                [],
    }
    ...
    for file_path in nifti_files:
        ...
        # ── Guard: warn loudly if any identifier could not be resolved ──────
        # Neither the filename nor the directory path yielded a usable token.
        # This should never happen for the expected
        #   <base>/sub-XXX/electrode_extraction/ses-N/run-MM/petra_inference.nii.gz
        # layout, but a renamed file or an unexpected folder structure would
        # land here. We flag it explicitly so such cases are never silently
        # written to the CSV with an UNKNOWN id.
        if 'UNKNOWN' in tag:
            counters['unresolved_id'] += 1
            counters['unresolved_id_sub'].append(file_path)
            log.warn(
                f"UNRESOLVED ID  {tag}  —  could not parse subject/session/run "
                f"from: {file_path}",
                print_also=True,
            )
    ...
    summary = [
        ...
        "  ── id parsing ─────────────────────────────────",
        f"  Unresolved subject/ses/run : {counters['unresolved_id']}"
        + ("  ✓" if counters['unresolved_id'] == 0 else "  ← check paths!"),
        "  ── sanity check ───────────────────────────────",
    ]
    accounted = (
    ...
    invalid_categories = [
        ...
        ('split_recovery_success_sub',        '3-cluster split: fused electrodes split to valid 4-config'),
        ('split_recovery_failed_sub',         '3-cluster split: split failed, left as 3 (invalid)'),
    ]
...
if __name__ == "__main__":
    ...
    output_csv = os.path.join(
        Table_path,
        f"electrode_positions_Paper340_images_{datetime.now().strftime('%Y%m%d')}.csv"
    )
