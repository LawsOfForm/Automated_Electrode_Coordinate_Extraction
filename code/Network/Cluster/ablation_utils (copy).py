...
def build_common_argparser(description=None):
    ...
    p.add_argument(
        "--split-mode", choices=["subject", "legacy_image"], default="subject",
        help="'subject' (default): whole participants go to exactly one split "
             "-- no leakage. 'legacy_image': the original image-wise shuffle, "
             "for reproducing published results only.",
    )
    p.add_argument(
...
def run_experiment(
    *,
    ...
    if split_mode == "subject" or (folds and folds >= 2):
    ...
    SPLIT_MANIFEST["config"] = {
        ...
        "leakage_free": bool(split_mode == "subject" or (folds and folds >= 2)),
    }
    ...
    if loss_name == "dicefocal":
        ...
        raise ValueError(
            f"Unknown loss_name='{loss_name}'. Use 'dicefocal' or 'diceloss'."
        )
