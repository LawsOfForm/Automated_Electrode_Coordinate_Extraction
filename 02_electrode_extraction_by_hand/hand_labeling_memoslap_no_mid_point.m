...
%% adapted by Filip Niemann; Filip.Niemann@med.uni-greifswald.de, 19.06.2024
...
im1 = (uint8(mat2gray(squeeze(mean(idqs(3, :, :), 1))) * 255));
im2 = (uint8(mat2gray(squeeze(mean(idqs(2, :, :), 1))) * 255));
im3 = (uint8(mat2gray(squeeze(mean(idqs(1, :, :), 1))) * 255));
...
save_nii(make_nii(idqs(1:3, :, :)), strcat(path_output, 'idqs.nii.gz'));

maskvq = zeros(size(vqs));
maskvq(vqs ~= 0) = 1;

%save_nii(make_nii(maskvq(1:5, :, :)), strcat(path_output, 'maskvq.nii.gz'));
save_nii(make_nii(maskvq(1:3, :, :)), strcat(path_output, 'maskvq.nii.gz'));
