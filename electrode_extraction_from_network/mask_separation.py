...
for sub_dir in alive_it(sub_dirs):
    cylinder_masks_path = op.join(sub_dir, "petra_cut_pads_dil_6_inference.nii.gz")
    ...
    masks = fast_divide_mask_merge_nearby(
        mask
    ...
    for i, m in enumerate(masks):
        if np.sum(m) < MAX_ELEMENT_SIZE:
            sep_mask_path = op.join(sub_dir, f"mask_{i}_inference.nii.gz")
            save_nifti(m, sep_mask_path, mask_nifti)
            continue

        separate_masks = slow_divide_mask(m, MAX_ELEMENT_SIZE)
        #separate_masks = fast_divide_mask_merge_nearby(m)
        for j, separate_mask in enumerate(separate_masks):
            sub_sep_mask_path = op.join(sub_dir, f"mask_{i}_{j}_inference.nii.gz")
