...
def fast_divide_mask_merge_nearby(mask):
    """
    Divide a mask into smaller masks by finding no overlap in the x, y, or z
    direction.
    ...
    list[np.ndarray]
        A list of masks
    """
    # function fills zeros with ones if difference of gap is smaller then tresholdet pixel
    def merge_mask(proj,threshold):
        nonzero = np.nonzero(proj)
        diff = np.diff(nonzero)
        id_fill = np.where((diff > 1) & (diff <threshold))[1]

        for num,index in enumerate(nonzero[0][id_fill]):
            for i in range(1,6):
                if proj[nonzero[0][id_fill][num]+i]>0:
                    break
                else:
                    proj[nonzero[0][id_fill][num]+i]=1
                    #print(f'index {nonzero[0][id_fill][num]+i} should be changed in proj to 1')
        return proj
    
    threshold=4 # close gap of 4 pixel
    ...
    x_proj_2=merge_mask(x_proj,threshold)   #x_proj is also changed after return of function, why ever
    y_proj_2=merge_mask(y_proj,threshold)
    z_proj_2=merge_mask(z_proj,threshold)

    x_nonzero = np.nonzero(x_proj_2)
    y_nonzero = np.nonzero(y_proj_2)
    z_nonzero = np.nonzero(z_proj_2)
